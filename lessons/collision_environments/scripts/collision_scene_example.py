...
class CollisionSceneExample(object):
    ...
    def add_four_boxes(self):
        box1_pose = [0.20, 0.50, 0.9, 0, 0, 0, 1]
        box1_dimensions = [0.2, 0.2, 0.5]

        box2_pose = [-0.55, -0.55, 0.9, 0, 0, 0, 1]
        box2_dimensions = [0.25, 0.25, 1.75]

        box3_pose = [0.4, -0.2, 1.0, 0, 0, 0, 1]
        box3_dimensions = [0.28, 0.28, 0.22]

        box4_pose = [-0.4, 0.4, 0.9, 0, 0, 0, 1]
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) != 2):
        print "Correct usage:: \n\"rosrun mara_moveit_tutorials collision_scene_example.py cluttered\" OR \n\"rosrun mara_moveit_tutorials collision_scene_example.py sparse\""
        sys.exit()
